...
2024 年，Princeton 团队做了一个实验：把 GPT-4 接入 SWE-bench（一个真实的 GitHub Issue 修复基准），模型没换，训练数据没动，只有一件事不同——接入模型的那套接口，重新设计了一遍。

结果：通过率从 3.8% 跳到了 12.5%。

同年，Anthropic 把 Claude 3.5 Sonnet 接入一套精简框架——两个工具（Bash + Edit）加一条约束（强制绝对路径）。SWE-bench Verified 得分：49%。

同一个模型，从 3.8% 到 49%。中间隔的不是更好的大脑，而是更好的身体结构。

这个差异指向一个被低估的事实：Agent 不是"一个聊天机器人加了几个工具"。它是一个控制系统。它的核心问题不是"怎么生成更好的回答"，而是"怎么在开放环境中持续完成目标"。
...
2023 年，一篇名为 CoALA（Cognitive Architectures for Language Agents）的论文试图回答一个问题：所有 LLM Agent 的共同结构是什么？
...
这三者组合起来，构成一个完整的认知架构。任何 Agent 系统——不管它用什么框架、什么模型——都可以用这套词汇来描述和分析。

### 1.2 四层映射

CoALA 的形式化与控制论的四层闭环之间，存在清晰的对应：
...
四层中每一层都有独立的工程问题，也有跨层的耦合关系。后面会逐层展开，最后再讲耦合。

### 1.3 Workflow vs Agent：什么时候用哪个

Anthropic 在工程指南中做了一个关键区分：

**Workflow**（编排型）：LLM 和工具通过预定义的代码路径编排。步骤是确定的，开发者写死了流程。适合定义清晰的任务。

**Agent**（自主型）：LLM 动态决定自己的执行路径和工具使用。步骤不可预测，模型自己选择下一步。适合开放式问题。

决策原则：从最简单的方案开始。Workflow 提供可预测性和一致性；Agent 提供灵活性，但代价是更高的成本、更难调试、错误会复利累积。只在复杂度确实需要时才升级到 Agent。
...
大多数生产系统不需要"Agent"。用 Workflow 能解决的问题，就不要用 Agent。
...
模型不能直接"看见"环境。它看见的是你喂给它的文本。感知层的设计决定了模型能理解什么、会忽略什么、在哪里犯错。

### 2.1 观察格式设计：Agent-Computer Interface

SWE-agent 的实验揭示了一个反直觉的事实：同一个模型，仅仅改变它与环境交互的接口格式，性能就能翻倍。

这个发现催生了一个概念——**ACI（Agent-Computer Interface）**。就像 HCI 为人类设计界面，ACI 为 LLM 设计界面。两者的需求完全不同：人类需要视觉、空间、交互；LLM 需要文本、顺序、受限于上下文窗口。

三条设计原则：

**最小认知开销**。用模型在训练数据中自然见过的格式。Markdown 表格比自定义 XML 好。带行号的代码比纯代码好。结构化错误信息比 raw stack trace 好。

**先给上下文，再要求精确输出**。不要上来就问"第 47 行应该改成什么"——先展示第 40-55 行的代码，然后再问。模型需要足够的 token 来"看清楚"才能给出精确答案。

**防错设计（Poka-yoke）**。强制绝对路径，消灭相对路径错误。用枚举参数替代自由文本。在工具定义里写清楚"不要做 X"。Anthropic 在 SWE-bench 开发中发现：花在工具接口设计上的时间，比花在 prompt 上的时间更有价值。

一个具体的例子：文件浏览。给模型看整个 2000 行文件？它会迷失在中间。给它一个分页查看器，每次 100 行，带行号，带当前位置标记？它能准确定位问题。这就是 ACI 设计的差距。

### 2.2 记忆架构：三层模型

Lilian Weng 在她的经典综述中提出了 Agent 记忆的三层模型，对应人类认知科学中的记忆分类：

**感知记忆**（Sensory Memory）——原始输入的第一层处理。对 Agent 来说，就是 API 返回的 raw JSON、终端输出的原始文本、截图的像素数据。持续时间极短，很快要么进入短期记忆，要么被丢弃。

**短期记忆**（Short-term / Working Memory）——上下文窗口。快但有限。这是模型当前能"想着"的全部信息。128K token 看起来很多，但对一个需要跨越几十个文件、几百次工具调用的长任务来说，很快就不够了。

**长期记忆**（Long-term Memory）——外部存储。向量数据库、文件系统、结构化数据库。容量无限但检索有延迟，且检索质量不稳定。
...
- **Filesystem as Context**：把工作状态写入文件，每次读取最新版本。文件系统本身就是一种外部记忆。Voyager 的技能库就是这个模式——成功的代码写入文件，按描述索引，下次检索复用。
- **Variables Manager**：显式追踪关键状态变量（当前目标、已完成步骤、待验证假设），而不是让模型从整个对话历史中隐式推断。
- **RAG**：语义检索。适合知识库查询，但对状态追踪不太适用。

### 2.3 上下文管理：对抗信息腐化

上下文窗口有一个不直觉的性质：不是"越长越好"。

Stanford 的"Lost in the Middle"研究发现，当关键信息落在长上下文的中间位置时，模型检索性能下降 30% 以上。即使窗口足够大，模型的注意力分布也不均匀——开头和结尾的信息更容易被注意到。

更严重的问题是：上下文超过窗口容量约 40% 后，性能开始急剧下降。不是线性衰减，是断崖式。Agent 在长任务中"跑着跑着就不对了"，很多时候不是因为推理能力退化，而是因为上下文已经腐化——充满了过时的工具输出、无关的中间步骤、早期的错误假设。
...
核心洞察：推理和行动必须交替进行。纯推理（Chain-of-Thought）会产生幻觉——模型编造不存在的事实。纯行动（无 Thought 直接调用工具）会盲目执行——模型不知道为什么要做这一步。ReAct 通过显式的 Thought 步骤迫使模型先说清楚"我为什么做这一步"，然后通过 Observation 用真实的环境反馈纠正推理。

实证：HotpotQA 上的幻觉率从 56% 降到接近 0%。
...
决策层的另一个核心能力是任务分解。一个复杂任务（"修复这个 bug"）如果不拆分，模型要在一个巨大的搜索空间中找到完整路径。拆分后（"1. 定位错误 2. 理解原因 3. 写修复 4. 写测试 5. 验证"），每一步的搜索空间大幅缩小。
...
**自动课程**（Voyager）。Agent 自己生成下一个目标。不需要人类预定义任务列表。Voyager 在 Minecraft 中根据当前状态和已有技能，自动生成"下一个值得尝试的事情"。这是最灵活的方式，但也最难控制。

**Routing**。不是分解成子步骤，而是根据输入类型分流到不同的专用路径。客服系统根据问题类型（退款/技术/咨询）路由到不同的处理 Agent。
...
一条拇指规则：如果你的 Agent 花了 30% 以上的 token 在"规划"上而不是"执行"上，规划可能过深了。把规划的粒度降低，让执行层的反馈来指导后续方向。
...
**1. 格式自然**。用模型训练数据中高频出现的格式。Markdown 比自定义 DSL 好。JSON 比 YAML 好（出现频率更高）。函数名用英文动词+名词（`search_files` 而不是 `fs_query_op`）。

**2. 文档完整**。工具描述不是给人看的，是给模型看的。它需要：一句话说清用途、参数的类型和约束、1-2 个使用示例、失败时返回什么、和相邻工具的区别（"用 `edit_file` 修改已有文件，用 `create_file` 创建新文件，不要混用"）。

**3. 防错设计**。强制绝对路径（消灭"当前目录是哪里"的歧义）。参数用枚举而不是自由文本（`mode: "append" | "overwrite"` 而不是 `mode: string`）。必填参数和可选参数明确区分。
...
一个反模式的量化数据：当可用工具超过 30 个时，模型的工具选择准确率开始明显下滑。改用动态加载（每步只暴露 Top 5-10 个最相关的工具），准确率提升约 30%。
...
**静态全量暴露**。简单，适合工具数 < 10 的场景。所有工具的 schema 始终在 prompt 里。优点是模型随时知道自己的全部能力；缺点是随着工具增加，prompt 越来越长，噪音越来越大。
...
**技能库**（Voyager）。这是最激进的方案：Agent 不仅使用预定义工具，还能创造新工具。Voyager 把执行成功的代码片段自动存入技能库，下次遇到类似任务时检索复用。工具集合随着 Agent 的经验增长而增长。
...
**权限最小化**（Constrained Tool Use）。Agent 只能用它完成当前任务所需的最小工具集。不需要删除文件的任务，就不暴露删除工具。

**不可逆操作确认**。删除、发布、支付——这类操作在执行前必须经过人类确认。不是因为 Agent 不可信，而是因为错误的代价不对称。
...
{"tool": "mine_block", "args": {"block_type": "diamond_ore"}}
```
...
代价是：需要更严格的沙箱。JSON 工具调用的安全边界很清晰（参数验证就够了）；代码执行的安全边界需要容器级别的隔离。
...
没有反馈的 Agent 就是一个开环系统——它不知道自己的行动产生了什么效果，只能盲目前进。错误会累积，一步错步步错。反馈层闭合了这个环路，让 Agent 能够感知行动的结果并据此调整。
...
**规则验证**。测试通过了吗？类型检查过了吗？Linter 有没有报错？这是最可靠的反馈——确定性的、无偏差的、成本几乎为零。如果你的任务有规则验证的条件（代码任务几乎都有），优先用它。
...
Reflexion 证明了自我反思的巨大价值——Agent 在失败后生成文本反思，存入记忆，下次避免同样的错误。HumanEval 从 67% 到 91%。
...
- **区分"信心低"和"确认出错"**：信心低但无外部证据表明出错 → 不反思。只有当外部反馈（测试失败、用户否定）明确表明出错时才触发反思。
...
**VIGIL**（2025）更进一步：运行时持续监控 Agent 行为，检测到异常（循环、延迟飙升、输出退化）时自动诊断并生成修复方案，而不需要等到任务完全失败。
...
设计原则：人类检查点的位置应该是预先设计好的，而不是"失败了就问人"。好的人机协作像是高速公路上的收费站——在关键节点短暂停下确认，而不是随时随地可能被打断。

---

## 6. 四层耦合：一个完整的例子

四层不是独立的。它们之间存在紧密的耦合关系。用一个代码修复 Agent 的完整执行过程来展示：
...
一条重要推论：**如果四层之间的接口设计不匹配，系统性能会远低于各层单独的能力**。一个优秀的决策层配上一个返回 raw dump 的感知层，整体表现会很差。这就是 SWE-agent 的实验结论——改善感知层的格式（ACI），在不改变决策层的情况下，整体性能翻了 3 倍。
...
1. **CoALA**（arXiv:2309.02427）— 最完整的 Agent 认知架构形式化。如果你只读一篇理论论文，读这篇。它提供了描述任何 Agent 系统的统一词汇。

2. **Anthropic: Building Effective Agents** — 最实用的工程原则。Workflow vs Agent 的选择、工具设计、何时不用 Agent。适合正在做架构决策的团队。
